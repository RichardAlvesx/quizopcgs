export default [
  {
    question: "Em qual cidade paranaense está localizado o contrato BOTICÁRIO-PR?",
    answers: [
      { option: "Curitiba - PR", correct: false },
      { option: "Campina Grande do Sul - PR", correct: true },
      { option: "Cidade Industrial - PR", correct: false },
    ],
  },
  {
    question: "Qual das alternativas está incorreta?",
    answers: [
      { option: "Fazem parte do Grupo Boticário: Eudora, Quem disse Berenice e Truss", correct: false },
      { option: "Fazem parte do Grupo Boticário: Eudora, Quem disse Berenice e Beleza na Web", correct: false },
      { option: "Fazem parte do Grupo Boticário: Eudora, Quem disse Berenice, Natura e Jequiti", correct: true },
    ],
  },
  {
    question: "Qual é o nosso nível atual em auditoria SCA?",
    answers: [
      { option: "Showcase", correct: false },
      { option: "Desenvolvimento", correct: false },
      { option: "Avançado", correct: true },
    ],
  },
  {
    question: "Qual a cidade e estado de origem da marca O Boticário?",
    answers: [
      { option: "Curitiba - PR", correct: true },
      { option: "Registro - SP", correct: false },
      { option: "Copacabana - RJ", correct: false },
    ],
  },
  {
    question: "Qual o ano de origem da marca O Boticário?",
    answers: [
      { option: "1877", correct: false },
      { option: "1987", correct: false },
      { option: "1977", correct: true },
    ],
  },
];
